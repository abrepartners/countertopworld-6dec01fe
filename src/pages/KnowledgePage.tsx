...
import { getKnowledgePageBySlug } from '../data/knowledgePages';
import SiteFooter from '../components/SiteFooter';

function PageFAQ({ items }: { items: { q: string; a: string }[] }) {
...
export default function KnowledgePage() {
  ...
  const page = slug ? getKnowledgePageBySlug(slug) : undefined;
  ...
  useEffect(() => {
    ...
    const cleanup = applyPageHead({
      ...
      ogImage: `https://countertopworldar.com/og/knowledge-${page.slug}.png`,
      ogType: 'article',
    });

    const script = document.createElement('script');
    script.type = 'application/ld+json';
    script.setAttribute('data-knowledge-schema', page.slug);
    const schemaData: Record<string, unknown>[] = [
      {
        ...
        headline: page.title,
        description: page.metaDescription,
        image: page.heroImage.startsWith('http') ? page.heroImage : `https://countertopworldar.com${page.heroImage}`,
        datePublished: page.publishDate,
        dateModified: page.updatedDate || page.publishDate,
        ...
        mainEntityOfPage: `https://countertopworldar.com/knowledge/${page.slug}`,
      },
    ];
  ...
  if (!page) return <Navigate to="/knowledge" replace />;
  ...
  return (
    <div className="min-h-screen bg-obsidian antialiased">
      <nav className="fixed top-0 inset-x-0 z-50 bg-obsidian/80 backdrop-blur-xl border-b border-stone-gold/10 px-6 h-14 flex items-center justify-between" aria-label="Site navigation">
        ...
        <Link to="/knowledge" className="flex items-center gap-1.5 text-[13px] text-cool-gray hover:text-vein-white transition-colors no-underline">
      ...
      <nav aria-label="Breadcrumb" className="pt-16 pb-0 px-6 bg-obsidian">
        <ol className="flex items-center gap-1.5 text-[12px] max-w-[720px] mx-auto">
          ...
          <li><Link to="/knowledge" className="text-cool-gray/60 hover:text-vein-white transition-colors no-underline">Knowledge Center</Link></li>
          <li><ChevronRight size={12} className="text-stone-gold/30" /></li>
          <li aria-current="page" className="text-stone-gold font-medium truncate max-w-[200px]">{page.title}</li>
        </ol>
      </nav>
      ...
      <header className="pt-8 pb-12 px-6 text-center">
        <div className="max-w-[720px] mx-auto">
          ...
          <h1 className="font-display text-[clamp(1.8rem,4vw,2.8rem)] font-light tracking-tight leading-tight text-vein-white mb-6">{page.title}</h1>
          <div className="flex items-center justify-center gap-4 text-[12px] text-cool-gray/60">
            <span className="flex items-center gap-1.5"><Calendar size={13} /> {new Date(page.publishDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
            <span className="text-stone-gold/20">·</span>
            <span className="flex items-center gap-1.5"><Clock size={13} /> {page.readTime}</span>
          </div>
        </div>
      </header>
      ...
      <div className="px-6 mb-12">
        <div className="max-w-[720px] mx-auto aspect-[2/1] rounded-[12px] overflow-hidden border border-stone-gold/10">
          <img src={page.heroImage} alt={page.heroImageAlt} className="w-full h-full object-cover brightness-[0.6]" loading="eager" />
        </div>
      </div>
      ...
      <main className="max-w-[720px] mx-auto px-6 pb-20">
        ...
        {page.sections.map((section, i) => (
          <section key={i} className="mb-10">
            ...
            <div
              className="text-[15px] text-cool-gray font-light leading-[1.8] space-y-4 [&_strong]:text-vein-white [&_strong]:font-medium [&_table]:w-full [&_table]:my-4"
